...
# ① 边界损失权重。做消融时把它改成 0，即退回原始 BCE+Dice 基线，便于对比。
BD_WEIGHT = 0.5
...
def boundary_loss(logits, target, normalize=True):
    ...
    Args:
        ...
        normalize: 是否按病灶平均深度归一化，使损失量级与 BCE/Dice 相当，
                   这样 BD_WEIGHT 取 0.5 左右即可；若改成 False（完全忠于官方
                   未归一化版本），BD_WEIGHT 需降到 0.005~0.01。
    """
...
def hybrid_loss(predictions, target,device):
    ...
    for prediction in predictions:
       ...
       # dice = lovasz_softmax(prediction, target)
        ...
        # ① 在原 BCE+Dice 基础上叠加轮廓距离损失（深监督的每一级输出都加）
        bd = boundary_loss(prediction, target)

        loss += bce + dice + BD_WEIGHT * bd
